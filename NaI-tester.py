...
if not os.path.exists('Data/CAMIS-Detector-Testing/testing-results.csv'):
    with open ('Data/CAMIS-Detector-Testing/testing-results.csv','a') as csv_file:
        ...
        writer_obj.writerow(['NaI ID', 'PMT ID', 'Gross Counts', 'Net Counts', 'Cs137 Peak Ch', 'Cs137 FWHM [ch]'])
...
PMT_ID = int(input('\nWhat is the new ID # on the PMT Base?  '))
...
results_list = [NaI_ID, PMT_ID, np.sum(counts[i_low:i_high]), np.sum(gauss_new(x_bins, *gf)), gf[1], 2*np.log(2)*gf[2]]
