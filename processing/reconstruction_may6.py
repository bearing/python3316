...
def compute_mlem_full(sysmat, counts, dims,  # env_dims, shield_dims,
    ...
    recon_img_previous = np.ones(recon_img.shape)  # TODO: Allow input of initial guess so that you can pause and
    # print MLEM iterations
    ...
    print("Check Level:", counts.sum() * 0.001 + 100)
    print("Standard Deviation:", np.std(counts))
    while itrs < nIterations and (diff.sum() > (0.001 * counts.sum() + 100)):
        sumKlamb = sysmat.dot(recon_img)  # TODO: Exception for zero values
        outSum = (sysmat * measured[:, np.newaxis]).T.dot(1/sumKlamb)
        recon_img *= outSum / sensitivity  # TODO: Exception for zero values
...
def image_reconstruction_full(sysmat_file, data_file,
                              ...
                              pxl_sze=(2, 10), edge_mask=False, mod_edges_to_mask=None, sides_interp=False,
    ...
    # tot_pxls = np.product(np.array(dims), axis=0) # TODO: For your convenience, tot_pxls
    ...
    proj_masked = np.ones([48, 48])  # TODO: clumsy
    sysmat_mask = np.ones(48 * 48)
    if edge_mask:  # TODO: Mask out elements you do not need in counts and sysmat
        print("Hi! Edge mask seen")
        proj_masked, sysmat_mask = mask_edge_pixels(mods=mod_edges_to_mask)
        counts = counts[proj_masked.astype('bool')]  # (counts * proj_masked).ravel()
        sysmat = sysmat[sysmat_mask.astype('bool'), :]
    ...
    # print("Test, delete line 263. Proj_masked upper left:", proj_masked[:12,:12])
    return params, (proj_masked, sysmat_mask)
...
class Reconstruction(object):
    ...
    def mlem_reconstruct(self, data_file, recon_save_fname=None, **kwargs):
        # TODO: Allow current iteration to be used as initial guess for compute_mlem_full (to allow saving iterations)
        try:
            niter = kwargs['nIterations']
        except Exception as e:
            niter = 10
...
def split_image_and_project(sysmat_file, data_file, recon_image, section_width_x=None, batch=False, norm=True,
    ...
    if proj_mask is not None:
        ...
        print("proj_mask shape: ", proj_mask.shape)
        counts = counts * proj_mask
    ...
    print("Total Recon Counts: ", recon_image.sum())
...
def main():
    ...
    params, masks = image_reconstruction_full(sysmat_fname, data_6cm_filt,  # TODO: remember to put data file name in split
                                              ...
                                              # edge_mask=edge_mask,
                                              # mod_edges_to_mask=mod_edges_to_mask
                                              )
    obj_params = params[0]
    ...
    split_image_and_project(sysmat_fname, data_6cm_filt, obj_params[0],
                            section_width_x=[40, 121, 40],
                            norm=True,
                            proj_mask=masks[0]  # this should be projection mask from mlem
                            )
    # TODO: 3d image recon
